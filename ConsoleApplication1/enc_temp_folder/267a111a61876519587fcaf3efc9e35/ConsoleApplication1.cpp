...
int main()
{
	...
	*/
	...
	while (++ var_int_pi < 2) { // 修改外联静态变量的值， 和别名以及引用类似的操作
		excuteVariableScope();
	}
	static float VAR_PI; // static 声明的变量只能在自己文件的作用域中
	cout << "excuteVariableScope  extern static float VAR_PI --> " << VAR_PI << endl; // 3
	excuteNewOperator();
